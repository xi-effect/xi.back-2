...
from app.common.config import ALLOWED_FILE_EXTENSIONS, FILE_STORAGE_PATH, MAX_FILE_SIZE
...
async def test_upload_files(
    ...
    assert_nodata_response(
        authorized_client.post(
            "/api/protected/file-service/files", files=files_data["files"]
        ),
    )
...
async def test_upload_file_conflict(
    ...
    assert_response(
        authorized_client.post(
            "/api/protected/file-service/files",
...
async def test_upload_file_extension_not_allowed(
    ...
    assert_response(
        authorized_client.post(
            "/api/protected/file-service/files", files=[file_to_upload]
        ),
        expected_code=415,
...
async def test_retrieve_file(
    ...
    await create_file(active_session, file_data["filename"], file_bytes)

    response = authorized_client.get(
        f"/api/protected/file-service/files/{file_data['filename']}",
    )
...
async def test_retrieve_file_not_found(
    ...
    assert_response(
        authorized_client.get(
            f"/api/protected/file-service/files/{faker.file_name}",
        ),
        expected_code=404,
...
async def test_delete_file(
    ...
    await create_file(active_session, file_data["filename"])
    ...
    async with active_session():
        assert not Path(FILE_STORAGE_PATH, file_data["filename"]).exists()
...
async def test_update_file(
    ...
    updated_filename = faker.file_name(extension=choice(ALLOWED_FILE_EXTENSIONS))
    updated_file_bytes = faker.binary(length=1)
    ...
    assert (Path(FILE_STORAGE_PATH, updated_filename)).exists()
    assert not (Path(FILE_STORAGE_PATH, file_data["filename"])).exists()

    with Path(FILE_STORAGE_PATH, updated_filename).open("rb") as f:
