...
async def test_legacy_ydoc_access_level_retrieving(
    ...
    assert_response(
        authorized_internal_client.get(
            ...
            headers={"X-Storage-Token": ydoc_access_content_token},
        ),
...
async def test_legacy_ydoc_access_level_invalid_token(
    authorized_internal_client: TestClient,
    content_token: str,
    ydoc_id: UUID,
) -> None:
    assert_response(
        authorized_internal_client.get(
            ...
            headers={"X-Storage-Token": content_token},
        ),
...
async def test_legacy_ydoc_access_level_retrieving_proxy_authorization_missing(
...
async def test_legacy_ydoc_content_retrieving(
    internal_client: TestClient,
    ydoc: YDoc,
) -> None:
...
@freeze_time()
async def test_legacy_ydoc_content_updating(
    faker: Faker,
    ...
    assert_nodata_response(
        internal_client.put(
            ...
            f"/ydocs/{personal_material.main_ydoc_id}/content/",
            content=content,
...
@freeze_time()
async def test_legacy_ydoc_content_clearing(
    active_session: ActiveSession,
    internal_client: TestClient,
    personal_material: PersonalMaterial,
) -> None:
...
async def test_legacy_ydoc_not_finding(
    faker: Faker,
    authorized_internal_client: TestClient,
