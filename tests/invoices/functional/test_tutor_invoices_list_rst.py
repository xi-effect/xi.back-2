...
async def test_tutor_invoices_listing(
    ...
    index_cursor: int | None,
    limit: int,
) -> None:
    cursor_created_at: datetime = (
        recipient_invoices_data[-1][0].created_at
        if index_cursor is None
        else recipient_invoices_data[index_cursor][0].created_at
    )
    cursor_recipient_invoice_id: int = (
        recipient_invoices_data[-1][0].id
        if index_cursor is None
        else recipient_invoices_data[index_cursor][0].id
    )
    cursor: dict[str, Any] | None = (
        None
        if index_cursor is None
        else {
            "created_at": cursor_created_at.isoformat(),
            "recipient_invoice_id": cursor_recipient_invoice_id,
        }
    )
    ...
    filtered_recipient_invoices_data = [
        ...
        if index_cursor is None
        or recipient_invoice.created_at < cursor_created_at
        or (
            recipient_invoice.created_at == cursor_created_at
            and recipient_invoice.id > cursor_recipient_invoice_id
        )
    ]
