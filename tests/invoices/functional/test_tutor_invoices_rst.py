...
async def test_recipient_invoice_updating_payment_status(
    ...
    assert_nodata_response(
        tutor_client.patch(
            f"/api/protected/invoice-service/roles/tutor/recipient-invoices/{recipient_invoice.id}/status/"
        )
    )

    async with active_session():
        assert (
            await RecipientInvoice.find_first_by_id(recipient_invoice.id)
        ).status == PaymentStatus.COMPLETE  # type:ignore[union-attr]   # invoice exists
