...
from tests.invoices.factories import RecipientInvoicePaymentTypeFactory
...
async def test_student_recipient_invoice_payment_confirmation(
    ...
    assert_nodata_response(
        student_client.post(
            f"/api/protected/invoice-service/roles/student/recipient-invoices/{recipient_invoice.id}/payment-confirmation/sender/",
    ...
    async with active_session() as session:
        ...
        assert_contains(
            recipient_invoice,
            {**payment_type_data, "status": PaymentStatus.WF_RECEIVER_CONFIRMATION},
        )
...
async def test_student_recipient_invoice_invalid_confirmation_payment(
    active_session: ActiveSession,
    student_client: TestClient,
    recipient_invoice: RecipientInvoice,
