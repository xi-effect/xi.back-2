...
async def test_tutor_tutorships_listing(
    ...
    assert_response(
        mub_client.get(
            f"/mub/tutor-service/tutors/{tutor_user_id}/students/",
...
async def test_student_tutorships_listing(
    ...
    assert_response(
        mub_client.get(
            f"/mub/tutor-service/students/{student_user_id}/tutors/",
            params=remove_none_values(
                {
                    "created_before": (
                        None
...
async def test_tutorship_creation(
    ...
    assert_response(
        mub_client.post(
            f"/mub/tutor-service/tutors/{tutor_user_id}/students/{student_user_id}/",
        ),
...
async def test_tutorship_creation_target_is_the_source(
    ...
    assert_response(
        mub_client.post(
            f"/mub/tutor-service/tutors/{tutor_user_id}/students/{tutor_user_id}/",
        ),
...
async def test_tutorship_creation_tutorship_already_exists(
    ...
    assert_response(
        mub_client.post(
            f"/mub/tutor-service/tutors/{tutorship.tutor_id}/students/{tutorship.student_id}/",
        ),
        expected_code=status.HTTP_409_CONFLICT,
...
async def test_tutorship_not_finding(
    ...
    assert_response(
        mub_client.request(
            method=method,
            url=f"/mub/tutor-service/tutors/{tutor_user_id}/students/{student_user_id}/",
        ),
