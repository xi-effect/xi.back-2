...
async def test_individual_classroom_retrieving(
    ...
    assert_response(
        student_client.get(
            f"/api/protected/tutor-service/roles/student/classrooms/{individual_classroom.id}/",
        ),
...
async def test_group_classroom_retrieving(
    ...
    assert_response(
        student_client.get(
            f"/api/protected/tutor-service/roles/student/classrooms/{enrollment.group_classroom_id}/",
        ),
        expected_json=group_classroom_student_data,
    )


async def test_classroom_retrieving_access_denied(
    outsider_client: TestClient,
    any_classroom: AnyClassroom,
) -> None:
    assert_response(
        outsider_client.get(
            f"/api/protected/tutor-service/roles/student/classrooms/{any_classroom.id}/"
        ),
...
async def test_individual_classroom_retrieving_classroom_not_found(
...
async def test_group_classroom_retrieving_classroom_not_found(
    active_session: ActiveSession,
    student_client: TestClient,
    enrollment: Enrollment,
) -> None:
