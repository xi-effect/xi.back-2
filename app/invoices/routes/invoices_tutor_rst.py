...
@router.post(
    path="/roles/tutor/recipient-invoices/{recipient_invoice_id}/payment-confirmation/receiver/",
...
async def confirm_tutor_recipient_invoice_payment(
    ...
    if tutor_invoice.status is not PaymentStatus.WF_RECEIVER_CONFIRMATION:
        raise PaymentStatusResponses.INVALID_CONFIRMATION
    tutor_invoice.status = PaymentStatus.COMPLETE


@router.post(
    path="/roles/tutor/recipient-invoices/{recipient_invoice_id}/payment-confirmation/unilateral/",
    status_code=status.HTTP_204_NO_CONTENT,
    responses=PaymentStatusResponses.responses(),
...
async def confirm_tutor_recipient_invoice_payment_with_payment_type(
    tutor_invoice: TutorRecipientInvoiceByID,
    data: RecipientInvoice.PaymentTypeSchema,
) -> None:
    if tutor_invoice.status is not PaymentStatus.WF_SENDER_CONFIRMATION:
        raise PaymentStatusResponses.INVALID_CONFIRMATION
    tutor_invoice.payment_type = data.payment_type
    tutor_invoice.status = PaymentStatus.COMPLETE
