...
class InvoicesResponses(Responses):
    INVOICES_NOT_FOUND = status.HTTP_404_NOT_FOUND, "Invoces not found"


@with_responses(InvoicesResponses)
async def get_invoice_by_id(invoice_id: Annotated[int, Path()]) -> Invoice:
    invoice = await Invoice.find_first_by_id(invoice_id)
    if invoice is None:
        raise InvoicesResponses.INVOICES_NOT_FOUND
    return invoice
