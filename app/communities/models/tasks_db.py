...
class Task(Base):
    ...
    InputSchema = MappedModel.create(
        columns=[name, task_type, creation_date, opening_date, closing_date]
    )
    ...
    async def get_paginated(  # noqa: WPS211
        ...
        if ordering == TaskOrdering.CLOSING_DATE:
            stmt = stmt.order_by(cls.closing_date)
        if ordering == TaskOrdering.OPENING_DATE:
            stmt = stmt.order_by(cls.opening_date)
        if ordering == TaskOrdering.CREATION_DATE:
