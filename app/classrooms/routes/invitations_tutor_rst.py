...
@router.put(
    ...
    response_model=GroupInvitation.ResponseSchema,
    status_code=status.HTTP_201_CREATED,
    summary="Recreate a group tutor invitation for a group classroom by id",
)
async def refresh_group_invitation(
    group_classroom: MyTutorGroupClassroomByID,
    group_invitation: MyGroupInvitationByGroupID,
) -> GroupInvitation:
...
async def delete_group_invitation(group_invitation: MyGroupInvitationByGroupID) -> None:
    await group_invitation.delete()
