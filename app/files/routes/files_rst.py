...
from app.common.config import FILE_STORAGE_PATH
...
async def upload_files(files: ValidatedFiles) -> None:
    for uploaded_file in files:
        ...
        with Path(FILE_STORAGE_PATH, str(uploaded_file.filename)).open("wb") as f:
...
async def get_file(filename: str) -> FileResponse:
    file_path = Path(FILE_STORAGE_PATH, filename)
    file_db = await File.find_first_by_kwargs(filename=filename)
    if file_db is None or not file_path.exists():  # тут глянуть условие
...
async def update_file(filename: str, file: ValidatedFile) -> None:
    ...
    if file_db is None:
        raise FileResponses.FILE_NOT_FOUND
    Path(FILE_STORAGE_PATH, filename).unlink()
    with Path(FILE_STORAGE_PATH, str(file.filename)).open("wb") as f:
